 ...
 * This class has LinkedList to keep buildings for 2 sides of the street.
 * @author Mert Gürşimşir
 * @version 1.0 (March 19 2022)
 */
@SuppressWarnings("unchecked") // for unchecked type warning.
public class StreetLinkedList extends City{
	private LinkedList<Building> street0;
	private LinkedList<Building> street1;

	
	/**
	 *	Constructor of StreetLinkedList with lengthValue.
	 * 	Assign LinkedList's length to parameter.
	 *  @param lengthValue give length to street 
	 */
	public StreetLinkedList(int lengthValue) throws Exception{
		super(lengthValue);
		street0 = new LinkedList<Building>();
		street1 = new LinkedList<Building>();
	...
	public void silhouette(){
		...
		LinkedList<Building> streetMaxHeight;
		if (maxHeightSide == 0) streetMaxHeight = (LinkedList<Building>)street0.clone();
		else streetMaxHeight = (LinkedList<Building>)street1.clone();

		//2D LinkedList that demonstrates the street (maxHeight x lengthOfTheStreet)
		LinkedList<LinkedList<Character>> cityArray = new LinkedList<LinkedList<Character>>();    

		for (int i = 0; i < streetMaxHeight.get(maxHeightPosition).getHeight(); ++i){
			for (int j = 0; j < getLength(); ++j){
				cityArray.add(new LinkedList<Character>());
				cityArray.get(i).add(j,' ');
			}
		}

		//Adding roofs of the buildings to the LinkedList
		for (int i = 0; i < getLength(); ++i){
			if (street0.get(i) != null){
				if (street1.get(i) != null){
		...
		//Adding right edges of the buildings to the LinkedList
		for (int i = 0; i < streetMaxHeight.get(maxHeightPosition).getHeight(); ++i){
			for (int j = 0; j < getLength(); ++j){
		...
		//Adding left edges of the buildings to the LinkedList
		for (int i = 0; i < streetMaxHeight.get(maxHeightPosition).getHeight(); ++i){
			for (int j = 0; j < getLength(); ++j){
				isFree = true;
				if (cityArray.get(i).get(j) == '-'){
	...
	public boolean equals(Object obj){
		StreetLinkedList other = (StreetLinkedList) obj;
	 ...
	 */
	public StreetLinkedList clone(){
		try{
			StreetLinkedList copy = (StreetLinkedList) super.clone();
			copy.street0 = (LinkedList<Building>)street0.clone();
			copy.street1 = (LinkedList<Building>)street1.clone();
			return copy;
		}
		catch(Exception e){
			return null;
