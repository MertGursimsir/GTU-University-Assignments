 ...
 * This class has LDLinkedList to keep buildings for 2 sides of the street.
 * @author Mert Gürşimşir
 * @version 1.0 (March 19 2022)
 */
@SuppressWarnings("unchecked") // for unchecked type warning.
public class StreetLDLinkedList extends City{
	private LDLinkedList<Building> street0;
	private LDLinkedList<Building> street1;
	
	/**
	 *	Constructor of StreetLDLinkedList with lengthValue.
	 * 	Assign list's length to parameter.
	 *  @param lengthValue give length to street 
	 */
	public StreetLDLinkedList(int lengthValue) throws Exception{
		super(lengthValue);
		street0 = new LDLinkedList<Building>();
		street1 = new LDLinkedList<Building>();
	...
	public void silhouette(){
		...
		LDLinkedList<Building> streetMaxHeight;
		if (maxHeightSide == 0) streetMaxHeight = (LDLinkedList<Building>)street0.clone();
		else streetMaxHeight = (LDLinkedList<Building>)street1.clone();

		//2D list that demonstrates the street (maxHeight x lengthOfTheStreet)
		LDLinkedList<LDLinkedList<Character>> cityArray = new LDLinkedList<LDLinkedList<Character>>();    

		for (int i = 0; i < streetMaxHeight.get(maxHeightPosition).getHeight(); ++i){
			for (int j = 0; j < getLength(); ++j){
				cityArray.addLast(new LDLinkedList<Character>());
				cityArray.get(i).add(j,' ');
			}
		}

		//Adding roofs of the buildings to the list
		for (int i = 0; i < getLength(); ++i){
			if (street0.get(i) != null){
				if (street1.get(i) != null){
		...
		//Adding right edges of the buildings to the list
		for (int i = 0; i < streetMaxHeight.get(maxHeightPosition).getHeight(); ++i){
			for (int j = 0; j < getLength(); ++j){
		...
		//Adding left edges of the buildings to the list
		for (int i = 0; i < streetMaxHeight.get(maxHeightPosition).getHeight(); ++i){
			for (int j = 0; j < getLength(); ++j){
				isFree = true;
				if (cityArray.get(i).get(j) == '-'){
	...
	public boolean equals(Object obj){
		StreetLDLinkedList other = (StreetLDLinkedList) obj;
	...
	public StreetLDLinkedList clone(){
		try{
			StreetLDLinkedList copy = (StreetLDLinkedList) super.clone();
			copy.street0 = (LDLinkedList<Building>)street0.clone();
			copy.street1 = (LDLinkedList<Building>)street1.clone();
			return copy;
		}
		catch(Exception e){
			return null;
