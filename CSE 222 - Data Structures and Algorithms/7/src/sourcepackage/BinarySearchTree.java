...
public class BinarySearchTree<E> extends BinaryTree<E> implements SearchTree<E>{
	...
	/**
	 * Method to convert current BST to an AVL tree.
	 */
	public void convertAVL(){
		//Balancing children nodes of the root recursively
		convertAVL(root);

		//Balancing the root
		if (balance(root) < 0) root = rotateRight(root);
		else if (balance(root) > 0) root = rotateLeft(root);

	}

	/**
	 * Recursive method for the conversion
	 * @param node Current node
	 */
	private void convertAVL(Node<E> node){
		if (node == null) return;

		//Balancing the left child
		if (balance(node.getLeft()) < -1) node.left = rotateRight(node.left);
		else if (balance(node.getLeft()) > 1) node.left = rotateLeft(node.left);
		
		//Balancing the right child
		if (balance(node.getRight()) < -1) node.right = rotateRight(node.right);
		else if (balance(node.getRight()) > 1) node.right = rotateLeft(node.right);
		
		//Balancing descendants
		convertAVL(node.getLeft());
		convertAVL(node.getRight());
	}

	/**
	 * Method to calculate height of the current node
	 * @param node Current node
	...
	/**
	 * Method to calculate balance of the current node
	 * @param node Current node
